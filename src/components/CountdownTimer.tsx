...
export default function CountdownTimer({
  ...
  useEffect(() => {
    // Reception start target: October 13, 2026 15:00 WITA (UTC+8)
    const targetDate = new Date(`${targetDateStr}T15:00:00+08:00`);
  ...
  const handleAddToGoogleCalendar = () => {
    const startDate = "20261013T000000Z"; // 08:00 WITA = 00:00 UTC
    const endDate = "20261013T090000Z";
    ...
    >
      ...
      {timeLeft.isPast ? (
        ...
        <div className="grid grid-cols-4 gap-2.5 sm:gap-3 mb-6">
          {/* Days */}
          <div className="flex flex-col items-center justify-center p-3 sm:p-3.5 rounded-2xl bg-gradient-to-b from-white to-[#F7F2E6] border border-[#d8c49e] shadow-xs">
            <span className="text-2xl sm:text-3xl font-bold font-serif-royal text-[#4a3b1a]">
              {String(timeLeft.days).padStart(2, "0")}
            </span>
            <span className="text-[10px] font-bold text-[#8a6d32] tracking-widest uppercase mt-1">
              HARI
            </span>
          </div>
      ...
      {/* Add to Calendar Action Button - Elegant Warm Gold Button */}
      <motion.button
        whileHover={{ scale: 1.02 }}
        whileTap={{ scale: 0.98 }}
        onClick={handleAddToGoogleCalendar}
        className="w-full inline-flex items-center justify-center px-6 py-3.5 rounded-full bg-gradient-to-r from-[#c5a059] via-[#b89855] to-[#9e7d3b] text-white font-bold text-xs tracking-widest uppercase shadow-md hover:brightness-105 transition-all cursor-pointer border border-[#d8c49e]"
      >
